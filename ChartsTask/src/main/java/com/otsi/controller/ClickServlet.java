...
public class ClickServlet extends HttpServlet {
	...
	// getCharts method...
	public Map<String, Object> getCharts(String chartName, String fieldName, String clickValue) {
		...
		if (fieldName != null && clickValue != null) {
			qb = QueryBuilders.boolQuery().must(QueryBuilders.matchQuery(fieldName, clickValue));
		}
		// searchResponse object...
		searchResponse = client.prepareSearch(chartMap.get("index_name").toString())
				.setTypes(chartMap.get("index_type").toString()).setQuery(qb)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String clickValue = request.getParameter("clickValue");
		...
		out.println("[");
		for (int i = 0; i < allids.length; i++) {
			map = getCharts(allids[i], fieldName, clickValue);
